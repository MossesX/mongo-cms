...
CREATE TABLE IF NOT EXISTS `core_block_scheme` (
  ...
  KEY `fk_core_block_scheme_core_area` (`core_area_id`)
...
CREATE TABLE IF NOT EXISTS `core_site` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_core_site_core_template` (`core_template_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=2 ;
...
ALTER TABLE `core_block_scheme`
  ADD CONSTRAINT `fk_core_block_scheme_core_area` FOREIGN KEY (`core_area_id`) REFERENCES `core_area` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
--
-- Ограничения внешнего ключа таблицы `core_site`
--
ALTER TABLE `core_site`
  ADD CONSTRAINT `fk_core_site_core_template` FOREIGN KEY (`core_template_id`) REFERENCES `core_template` (`id`);
